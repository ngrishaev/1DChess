...
namespace Model
{
    ...
    public static class PathAvailabilityServiceStatic
    {
    ...
    public class PathAvailabilityService
    {
        public bool IsStraightPathAvailable(int from, int to, List<Piece> pieces) =>
            !pieces.Any(otherPiece => InBetweenStraight(otherPiece, from, to));

        public bool IsDiagonalPathAvailable(int from, int to, List<Piece> pieces)
        {
            if (from % 2 != to % 2)
                return false;
        ...
        public bool IsKingPathAvailable(int from, int to) =>
            to == from + 1 || to == from - 1;

        public bool IsKnightPathAvailable(int positionValue, int newPosition) =>
            positionValue == newPosition - 2 ||
            positionValue == newPosition - 3 ||
            positionValue == newPosition + 3 ||
            positionValue == newPosition + 2;

        public bool IsPawnPathAvailable(int from, int to, bool pawnAlreadyMoved, bool rightIsForward, List<Piece> pieces)
        {
            var firstPosition = rightIsForward ? from + 1 : from -1; 
            if (to == firstPosition)
                return true;
            ...
            if (to != secondPosition || pawnAlreadyMoved)
                return false;
            return IsPawnPathAvailable(from, to, true, rightIsForward, pieces) ||
                   IsStraightPathAvailable(from, to, pieces);
        }
